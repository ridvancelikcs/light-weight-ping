...
int send_ping(char *ip)
{
	...
	int ttl_val = 128;
	if (setsockopt(ping_sockfd, SOL_IP, IP_TTL, &ttl_val, sizeof(int)) != 0)
	{
	...
	tv_out.tv_usec = 0;
	if (setsockopt(ping_sockfd, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv_out, sizeof(struct timeval)) != 0)
	{
	...
	if (sendto(ping_sockfd, &x_pckt, sizeof(x_ping_pckt_t), 0, (struct sockaddr *)&ping_addr, sizeof(struct sockaddr_in)) <= 0)
	{
	...
	int 				recv_ret = recvfrom(ping_sockfd, &r_pckt, sizeof(r_ping_pckt_t), 0, NULL, NULL);
